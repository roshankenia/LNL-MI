...
def low_loss_over_epochs_labels(y_1, t, lowest_loss, indices, epoch, ite):
    ...
    if epoch % 5 == 0 and ite % 250 == 0:
        # create graph
        # make plot of entropy and peak value
        print('Making plot')
        result_df = pd.DataFrame(
            {'Entropy': entropyLoss.data.cpu(), 'Peak Value': peakValues, 'label': lowest_loss.noise_or_not[indices]})
    ...
    num_use = torch.nonzero(entropyLoss < 0.5*entropyLoss.mean()).shape[0]
    ...
    totalLoss = F.cross_entropy(clean_logits, clean_labels)
